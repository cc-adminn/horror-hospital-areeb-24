    ...
    public class DragRigidbody : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (InputHandler.InputIsInitialized && !gameManager.isPaused && !gameManager.isInventoryShown)
            {
                ...
                {
                    if (InputConverter.ReadButtonOnce(this, "Examine"))
                    {
                        GrabObject = !GrabObject;
                    }
                }

                if (InputConverter.ReadButtonOnce(this, "Zoom") && objectHeld)
                {
                    ThrowObject();
                }

                Vector2 delta = InputConverter.Look;
                ...
                if (InputHandler.CurrentDevice == InputHandler.Device.MouseKeyboard)
                    zoomInputY = InputConverter.Scroll.y * objectZoomSpeed;
                else
                    zoomInputY = InputConverter.Move.y * (RotateButton ? 1 : 0);
            }
            ...
            if (Physics.Raycast(playerAim, out RaycastHit hit, PickupRange, CullLayers))
            {
                ...
                if (objectHeld)
                {
                    if (RotateButton && enableObjectRotation)
                    {
                        ...
                        gameManager.LockPlayerControls(true, false, false, false);
                    ...
                    {
                        gameManager.LockPlayerControls(true, true, false, false);
        ...
        public void ResetDrag(bool throwObj)
        {
            ...
            gameManager.LockPlayerControls(true, true, true, false);
