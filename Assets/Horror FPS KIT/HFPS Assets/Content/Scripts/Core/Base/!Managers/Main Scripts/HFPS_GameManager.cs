...
#region Texts
        ...
        void Update()
        {
            ...
            if (InputConverter.ReadButtonOnce(this, "Inventory"))
            {
                OnInventory();
            }

            if (InputConverter.ReadButtonOnce(this, "Pause"))
            {
                OnPause();
            }
        }
        ...
        /// <param name="Mobile Input Rig">true = enabled, false = disabled</param>
        public void LockPlayerControls(bool MobileInputRig, bool Controller, bool Interact, bool CursorVisible, int BlurLevel = 0, bool BlurEnable = false, bool ResetBlur = false, int ForceLockLevel = 0)
        {
            if (ForceLockLevel == 2)
            {
                playerLocked = false;
        ...
        public void LoadNextScene(string scene)
        {
            if (saveHandler)
            {
                if (saveHandler.crossSceneSaving)
                {
                    ...
                    if (!isPaused)
                    {
                        LockPlayerControls(false, false, false, false);
                    }
